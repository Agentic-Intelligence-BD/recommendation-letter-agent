...
import { z } from 'zod';
import { X, Plus, Trash2, User, Mail, GraduationCap, BookOpen } from 'lucide-react';
...
export default function AddStudentModal({ isOpen, onClose, onAddStudent }: AddStudentModalProps) {
  ...
  const [colleges, setColleges] = useState<Array<{ name: string; type: string }>>([{ name: '', type: 'liberal-arts' }]);
  ...
  const onSubmit = (data: StudentFormData) => {
    ...
    setSubjects(['']);
    setActivities(['']);
    setColleges([{ name: '', type: 'liberal-arts' }]);
  };
